...
IF (USB_DYNAMIC_DESC_ENABLE_VERSION)
   _USB_DYNAMIC_VERSION::
   USB_DYNAMIC_VERSION::                BLK   2    ; Must be set prior to calling USB_Start
ENDIF
IF (USB_DYNAMIC_DESC_ENABLE_COUNTRY)
   _USB_DYNAMIC_COUNTRY::
   USB_DYNAMIC_COUNTRY::                BLK   1    ; Must be set prior to calling USB_Start
ENDIF
IF (USB_DYNAMIC_DESC_ENABLE_VID)
   _USB_DYNAMIC_VID::
   USB_DYNAMIC_VID::                    BLK   2    ; Must be set prior to calling USB_Start
ENDIF
IF (USB_DYNAMIC_DESC_ENABLE_PID)
   _USB_DYNAMIC_PID::
   USB_DYNAMIC_PID::                    BLK   2    ; Must be set prior to calling USB_Start
ENDIF
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; DYNAMIC DESCRIPTORS By BRW
   IF (USB_DYNAMIC_DESC_ENABLE_VERSION) ; Dynamic_Version
      cmp   [USB_DataPtr], >USB_D0_DESCR_START
      jnz   .Dynamic_Version_MSB  
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+13      
      jnz   .Dynamic_Version_MSB   
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_VERSION
      mov   A, [USB_DYNAMIC_VERSION+1]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead	      
   .Dynamic_Version_MSB:      
      cmp   [USB_DataPtr], >USB_D0_DESCR_START      
      jnz   .Dynamic_Country          
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+14      
      jnz   .Dynamic_Country      
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_VERSION
      mov   A, [USB_DYNAMIC_VERSION] 
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead     
   ENDIF      
   ...
   .Dynamic_Country:
   IF (USB_DYNAMIC_DESC_ENABLE_COUNTRY)
      cmp   [USB_DataPtr], >USB_D0_C1_I0_HID_DESCR_START
      jnz   .Dynamic_VID  
      cmp   [USB_DataPtr+1], <USB_D0_C1_I0_HID_DESCR_START+5
      jnz   .Dynamic_VID
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_COUNTRY
      mov   A, [USB_DYNAMIC_COUNTRY]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead	
   ENDIF
   ...
   IF (USB_DYNAMIC_DESC_ENABLE_VID)
      cmp   [USB_DataPtr], >USB_D0_DESCR_START
      jnz   .Dynamic_VID_MSB
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+9
      jnz   .Dynamic_VID_MSB
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_VID
      mov   A, [USB_DYNAMIC_VID+1]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead	
   .Dynamic_VID_MSB:
      cmp   [USB_DataPtr], >USB_D0_DESCR_START
      jnz   .Dynamic_PID
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+10
      jnz   .Dynamic_PID
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_VID
      mov   A, [USB_DYNAMIC_VID]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead
   ENDIF
   ...
   IF (USB_DYNAMIC_DESC_ENABLE_PID) 
      cmp   [USB_DataPtr], >USB_D0_DESCR_START
      jnz   .Dynamic_PID_MSB
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+11
      jnz   .Dynamic_PID_MSB
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_PID
      mov   A, [USB_DYNAMIC_PID+1]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
      jmp   .Done_DynamicVersionRead	
   .Dynamic_PID_MSB:
      cmp   [USB_DataPtr], >USB_D0_DESCR_START
      jnz   .Done_DynamicVersionRead
      cmp   [USB_DataPtr+1], <USB_D0_DESCR_START+12
      jnz   .Done_DynamicVersionRead
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_11b ; change to use cur_pp
      RAM_SETPAGE_CUR >USB_DYNAMIC_PID
      mov   A, [USB_DYNAMIC_PID]
      RAM_CHANGE_PAGE_MODE FLAG_PGMODE_00b ; change back to interrupt
   ENDIF
.Done_DynamicVersionRead:
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
USB_GET_DEVICE_TABLE_ENTRY:
    ...
    MOV     A,>Global_Device_Table_Lookup_Location  ; Get the ROM Address MSB
    MOV     X,<Global_Device_Table_Lookup_Location  ; Get the ROM Address LSB
    romx
...
.GetBootLoaderDevice:
    ...
    MOV     A,>USB_BTLDR_DEVICE_LOOKUP ; Get the ROM Address MSB
    MOV     X,<USB_BTLDR_DEVICE_LOOKUP ; Get the ROM Address LSB
.GetDeviceEntry:
    PUSH    A                          ; Save the MSB on the stack 
    ROMX                               ; First entry is the table size (only a byte)
    CMP     A, [USB_t2]                ; Range check
    POP     A	                         ; kvn -- get either of the two possible ROM addr MSB's 
    ;MOV     A,>USB_DEVICE_LOOKUP      ; Get the ROM Address MSB
    JC      .exit
...
IF (USB_CB_@0_@1_@2 & 1)
    LJMP     USB_DT_@0_@1_@2_Dispatch
	NOP
ELSE
    LJMP     USB_Not_Supported_Local_Drv
	NOP
ENDIF
ENDM
...
USB_DT_End:
USB_DT_Size: equ (USB_DT_End-USB_DT_bmRequestType) / 4
USB_bmRequestType_Dispatch::
    LJMP_DISPATCHER USB_DT_bmRequestType, USB_DT_Size, USB_Not_Supported_Local_Drv
